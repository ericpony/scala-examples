...
object lambdaDot {
  ...
  case class Rcd(ds: _root_.collection.List[Dec]) extends Type
  ...
  abstract class Decs
  case class FiniteDecs(l: _root_.collection.List[Dec]) extends Decs
  case class BotDecs() extends Decs
  
  def lookupInEnv(x: BigInt, env: _root_.collection.List[(BigInt, Type)]): Option[Type] = env match {
    case Cons((y, t), rest) => if (x == y) Some(t) else lookupInEnv(x, rest)
    case Nil() => None()
  }
  
  def lookupInDecList(name: Label, l: _root_.collection.List[Dec]): Option[Dec] = (l match {
  ...
  def wfType(env: _root_.collection.List[(BigInt, Type)], t: Type, shallow: Boolean): Boolean = t match {
    case Top() => true
    case Bot() => true
  ...
  def wfDec(env: _root_.collection.List[(BigInt, Type)], d: Dec): Boolean = d match {
    case TDec(l, lo, hi) => wfType(env, lo, false) && wfType(env, hi, false)
    case MDec(m, a, r) => wfType(env, a, false) && wfType(env, r, false)
  }
  
  def wfDecList(env: _root_.collection.List[(BigInt, Type)], ds: _root_.collection.List[Dec]): Boolean = ds match {
    case Cons(d, ds) => wfDec(env, d) && wfDecList(env, ds)
    case Nil() => true
  }

  def expansion(env: _root_.collection.List[(BigInt, Type)], t: Type): Option[Decs] = {
    //require(wfType(env, t, false))
    t match {
      case Top() => Some(FiniteDecs(_root_.collection.Nil()))
      case Bot() => Some(FiniteDecs(_root_.collection.Nil()))
  ...
  def lookupInVar(env: _root_.collection.List[(BigInt, Type)], x: BigInt, l: Label): Option[Dec] = {
