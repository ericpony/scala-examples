...
//some non-leon functions to test the program with scala
  ...
  def clause2list(cl: VarList): collection.List[Int] = cl match {
    case VarCons(v, vs) => v :: clause2list(vs)
    case VarNil() => collection.Nil
    case VarLit(b) => if(b) collection.List(1) else collection.List(-1)
  }
  def clauses2list(cll: ClauseList): collection.List[collection.List[Int]] = cll match {
    case ClauseCons(cl, cls) => clause2list(cl) :: clauses2list(cls)
    case ClauseNil() => collection.Nil
    case ClauseLit(b) => if(b) collection.List(collection.List(1)) else collection.List(collection.List(-1))
  }
}
