...
object Justify {
  ...
  def addSpaces(words: collection.List[BigInt], p: BigInt, q: BigInt, t: BigInt, n: BigInt, i: BigInt): collection.List[BigInt] = words match {
    case Nil() => collection.Nil()
    case Cons(head, tail) =>
      if(i <= t) collection.Cons(head + p*(i-1), addSpaces(tail, p, q, t, n, i+1))
      else if(i > t && i <= n) collection.Cons(head + p*(t-1) + q*(i-t), addSpaces(tail, p, q, t, n, i+1))
      else collection.Nil() //this should never happen
  }
  ...
  def justifyImpl(n: BigInt, z: BigInt, s: BigInt, words: collection.List[BigInt]): (BigInt, BigInt, BigInt, collection.List[BigInt]) = {
    require(n >= 2 && s >= 0 && z >= 1)
    val (q, t, p) = if(z % 2 == 0) {
      val tmp = s / (n-1)
      (tmp, 1 + (s % (n - 1)), tmp + 1)
