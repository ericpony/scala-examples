...
object Relations {
  sealed case class G[A](domain: collection.List[A])
  ...
  def setOf[A](l: collection.List[A]): (A => Boolean) = (l.contains(_))

  def test(r1: ((Int,Int))  => Boolean, r2: ((Int,Int))  => Boolean): Boolean = {
    implicit val g1: G[Int] = G[Int](collection.List(1,2))
    implicit val g2: G[(Int,Int)] = G(collection.List(1 -> 1, 1 -> 2, 2 -> 2, 2 -> 1, 3 -> 2, 3 -> 1))
    //val r1: ((Int,Int))  => Boolean = setOf(List(1 -> 2, 2 -> 3, 3 -> 1))
    //val r2: ((Int,Int))  => Boolean = setOf(List(1 -> 1, 2 -> 3, 3 -> 2))
    val commutative = 
      eq(circ(r1, r2), circ(r2, r1))

    val r3: ((Int,Int))  => Boolean = setOf(collection.List(1 -> 5, 2 -> 2, 1 -> 2))
    val associative =
