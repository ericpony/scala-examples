...
object Tree { 
  ...
  def nil[A]: collection.List[A] = collection.Nil[A]
  ...
  case class Path(p: collection.List[Dir]) extends AnyVal
  def pnil = Path(nil)
  ...
  def prefix(p1: Path, p2: Path): Boolean = {
    (p1.p, p2.p) match {
      case (Nil(),_) => true
      case (h1 collection.:: r1, h2 collection.:: r2) => {
  ...
  case class Flat(trees: collection.List[(Path, Tree)]) extends AnyVal
  ...
  def addLeft(f: Flat): Flat = {
    f.trees match {
      case Nil() => fnil
      case (p,t) collection.:: trees1 => Flat((Path(Left::p.p),t) :: addLeft(Flat(trees1)).trees)
    }
  }
  def addRight(f: Flat): Flat = {
    f.trees match {
      case Nil() => fnil
      case (p,t) collection.:: trees1 => Flat((Path(Right::p.p),t) :: addRight(Flat(trees1)).trees)
    }
  }
