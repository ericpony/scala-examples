...
object HeapSort {
  ...
  def listMax(l: collection.List[BigInt]): BigInt = {
    require(l != collection.Nil[BigInt]())
    l match {
      case Cons(x, Nil()) =>
        x
      case Cons(x, tail) =>
  ...
  def append(l1: collection.List[BigInt], l2: collection.List[BigInt]): collection.List[BigInt] = {
    l1 match {
      case Nil() => l2
      case Cons(x, tail) =>
        collection.Cons(x, append(tail, l2))
    }
  } ensuring (res => (l2 != collection.Nil[BigInt]() || res == l1) &&
    (res == collection.Nil[BigInt]() ||
      ((l1 == collection.Nil[BigInt]() && listMax(res) == listMax(l2)) ||
        (l2 == collection.Nil[BigInt]() && listMax(res) == listMax(l1)) ||
        (listMax(res) == max(listMax(l1), listMax(l2))))))

  def heapElements(t: Heap): collection.List[BigInt] = {
    ...
    (res != collection.Nil[BigInt]() && rootVal(t) == listMax(res)))
