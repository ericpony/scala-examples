...
object Robot {
  ...
  case class Position(x: BigInt, y: BigInt) {
    ...
    def neighbors: collection.List[Position] = {
      collection.List(this + North, this + East, this + South, this + West)
    }
  }
  ...
  def engineSucc(es: EngineState): collection.List[EngineTransition] = {
    if ((es.pos.x <= 0 && es.orient == West) ||
        (es.pos.y <= 0 && es.orient == North) ||
        (es.pos.x >= es.dim.x && es.orient == East) ||
        (es.pos.y >= es.dim.y && es.orient == South)) {
      collection.List(ERotateLeft, ERotateRight, EIdle)
    } else {
      collection.List(EForward, ERotateLeft, ERotateRight, EIdle)
    }
  }
  ...
  def navSensorSucc(ns: NavSensorState): collection.List[NavSensorTransition] = {
    if (ns.hasData) {
      collection.List(NForget, NIdle)
    } else {
      collection.List(NSense, NIdle)
    }
  }
  ...
  def heatSensorSucc(hs: HeatSensorState): collection.List[HeatSensorTransition] = {
    if (hs.hasData) {
      collection.List(HForget, HIdle)
    } else {
      collection.List(HSense, HIdle)
    }
  }
  ...
  def transmitterSucc(ts: TransmitterState): collection.List[TransmitterTransition] = {
    if (ts.hasData) {
      collection.List(TRepeat, TFound)
    } else {
      collection.List(TFound, TIdle)
    }
  }
  ...
  def robotSucc(rs: RobotState): collection.List[RobotTransition] = {
  ...
  def filterValid(rs: RobotState, rts: collection.List[RobotTransition]): collection.List[RobotTransition] = (rts match {
    case Cons(rt, rtt) =>
      val ts = filterValid(rs, rtt)

      if (validTransition(rs, rt)) {
        collection.Cons(rt, ts)
      } else {
        ts
      }
    case Nil() => collection.Nil[RobotTransition]()
  }) ensuring {
    res => allValid(rs, res)
  }

  def allValid(rs: RobotState, rts: collection.List[RobotTransition]): Boolean = rts match {
  ...
  def getBestTransition(rs: RobotState, ls: collection.List[RobotTransition]): Option[RobotTransition] = {
  ...
  def draw(w: World, ogt: Option[RobotTransition]): Unit = {
    ...
    val colors = collection.List(52, 124, 1, 160, 196);
  ...
  def ||(ets: collection.List[EngineTransition],
         nsts: collection.List[NavSensorTransition],
         hsts: collection.List[HeatSensorTransition],
         tts: collection.List[TransmitterTransition]): collection.List[RobotTransition] = {
